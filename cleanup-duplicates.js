...
if (duplicates.length > 0) {
  ...
  duplicates.forEach(dup => {
    ...
    toDelete.forEach(record => {
      ...
      db.prepare('DELETE FROM po_items WHERE po_number = ?').run(dup.po_number);

      // Delete the duplicate header
      db.prepare('DELETE FROM po_headers WHERE rowid = ?').run(record.rowid);

      console.log(`  Deleted duplicate record (rowid: ${record.rowid}, created: ${record.created_at})`);
    });

    // Re-insert items for the kept record
